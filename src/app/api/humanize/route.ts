...
export async function POST(req: NextRequest) {
  ...
  if (!apiKey) {
    return NextResponse.json(
      { error: 'Humanizer backend not configured. Please set ANTHROPIC_API_KEY environment variable.' },
      { status: 503 }
    )
  }
  ...
  try {
    ...
    const output = content && content.type === 'text' ? content.text.trim() : ''
...
export async function GET() {
  return NextResponse.json({
    ...
    max_chars: MAX_CHARS,
    languages: ['en', 'zh'],
    requires_api_key: !process.env.ANTHROPIC_API_KEY,
  })
}